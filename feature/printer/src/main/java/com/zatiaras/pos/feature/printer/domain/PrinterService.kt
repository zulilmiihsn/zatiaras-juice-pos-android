...
class PrinterService @Inject constructor(
     ...
     */
    fun isBluetoothReady(): Boolean = printerManager.isBluetoothAvailable() && printerManager.isBluetoothEnabled()

    /**
     * Get the name of connected printer, or null if not connected.
     */
    fun getConnectedPrinterName(): String? = printerManager.getConnectedDevice()?.displayName
    ...
    suspend fun printReceipt(receipt: Receipt): Result<Unit> {
        ...
        return try {
            ...
            Timber.d("Printing receipt ${receipt.number}")
