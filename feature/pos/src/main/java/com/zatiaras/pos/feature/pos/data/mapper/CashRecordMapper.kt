 ...
 */
fun CashRecordEntity.toDomain(): CashRecord = CashRecord(
    id = id,
 ...
 */
fun List<CashRecordEntity>.toDomainList(): List<CashRecord> = map { it.toDomain() }

/**
 * Convert domain CashRecord to CashRecordEntity.
 */
fun CashRecord.toEntity(): CashRecordEntity = CashRecordEntity(
    id = id,
    type = type.name,
    amount = amount,
    description = description,
