...
data class Cart(
     ...
     * @return New Cart with updated items
     */
    fun incrementItem(uniqueKey: String): Cart = copy(
        items = items.map { item ->
            if (item.uniqueKey == uniqueKey) {
     ...
     */
    fun removeItem(uniqueKey: String): Cart = copy(items = items.filter { it.uniqueKey != uniqueKey })
     ...
     */
    fun getItem(uniqueKey: String): CartItem? = items.find { it.uniqueKey == uniqueKey }

    /**
     * Gets all cart items for a specific product (regardless of customizations).
     */
    fun getItemsForProduct(productId: String): List<CartItem> = items.filter { it.product.id == productId }

    /**
     * Returns the total quantity of a specific product in cart
     * (across all customization variants).
     */
    fun getTotalQuantityForProduct(productId: String): Int = getItemsForProduct(productId).sumOf { it.quantity }
