...
class CashRecordRepositoryImpl @Inject constructor(
    ...
    override suspend fun getRecordById(id: String): CashRecord? = cashRecordDao.getById(id)?.toDomain()
    ...
    override fun getRecordsByDateRange(startDate: Long, endDate: Long): Flow<List<CashRecord>> = cashRecordDao.getByDateRange(startDate, endDate)
        .map { entities -> entities.toDomainList() }

    override suspend fun deleteRecord(id: String): Result<Unit> = try {
    ...
    override suspend fun getTodaySummary(): Result<CashSummary> = try {
        val (startOfDay, endOfDay) = DateUtils.getTodayRange()
