...
class UserRemoteDataSource @Inject constructor(
     ...
     */
    suspend fun fetchAllUsers(): Result<List<UserDto>> = try {
     ...
     */
    suspend fun fetchActiveUsers(): Result<List<UserDto>> = try {
        val users = postgrest.from(TABLE_PENGGUNA)
            .select(USER_PROFILE_COLUMNS) {
                filter {
                    eq("is_active", true)
     ...
     */
    suspend fun fetchActiveUserWithPassword(username: String): Result<UserDto?> = try {
     ...
     */
    suspend fun updatePasswordHash(userId: String, newPasswordHash: String): Result<Unit> = try {
