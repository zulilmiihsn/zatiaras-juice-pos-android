...
class SupabaseAuthRepository @Inject constructor(
    ...
    override fun isUserLoggedIn(): Flow<Boolean> = auth.sessionStatus.map { status ->
        status is io.github.jan.supabase.gotrue.SessionStatus.Authenticated
    }

    override suspend fun changeCurrentUserPassword(currentPassword: String, newPassword: String): Result<Unit> = Result.Error(UnsupportedOperationException("Change password is not supported in SupabaseAuthRepository"))
