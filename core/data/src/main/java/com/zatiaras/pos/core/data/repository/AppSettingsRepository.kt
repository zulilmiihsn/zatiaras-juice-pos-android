...
class AppSettingsRepository @Inject constructor(
     ...
     */
    suspend fun getStoreName(): String = settingsDao.getStoreName() ?: "Zatiaras Juice"

    /**
     * Get store address for receipt.
     */
    suspend fun getStoreAddress(): String? = settingsDao.getStoreAddress()

    /**
     * Get default paper width.
     */
    suspend fun getDefaultPaperWidth(): Int = getSettings()?.defaultPaperWidth ?: 58
     ...
     */
    suspend fun isOwnerPinSet(): Boolean = settingsDao.getOwnerPinHash() != null
     ...
     */
    suspend fun isRouteLocked(route: String): Boolean = getLockedRoutes().contains(route)
     ...
     */
    suspend fun toggleRouteLock(route: String): Result<Unit> = if (isRouteLocked(route)) {
        unlockRoute(route)
    } else {
        lockRoute(route)
    }
     ...
     */
    suspend fun getDefaultTaxPercentage(): Double = getSettings()?.defaultTaxPercentage ?: 0.0
