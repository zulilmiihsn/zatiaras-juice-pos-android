...
class SyncManagerTest {
    ...
    fun setup() {
        ...
        coEvery { cashRecordSyncer.sync() } returns SyncResult(
            syncType = SyncType.CASH_RECORDS,
            totalSynced = 3,
            failed = 0
        )
        
        syncManager = SyncManager(context, syncPreferences, transactionSyncer, cashRecordSyncer)
    }
    ...
    fun `syncNow returns results from all syncers`() = runTest {
        ...
        assertEquals(5, results[0].totalSynced)
        assertEquals(3, results[1].totalSynced)
    }
    ...
    fun `getSyncInfo combines all sync info`() = runTest {
        ...
        syncManager.getSyncInfo().collect { syncInfo ->
